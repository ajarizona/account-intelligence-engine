...
A company profile goes in (`examples/harborview_profile.json`, the Harborview Financial Group example). `engine.py` scores it against the objective layer, the vendor catalog, and the patterns layer, and produces a structured result (`examples/harborview_engine_result.json`): ranked use cases, the crawl/walk/run roadmap, the attached value pattern, the relevant execution challenges, and the industry's strategic tensions. From that same structured result, `render_readout.py` produces a readable markdown readout (`examples/harborview_generated_readout.md`), and `src/render_deck.py` produces a meeting-ready `.pptx` deck (`examples/harborview_readout_deck.pptx`), reachable from the CLI via `engine.py --out-deck`. The deck follows the same crawl/walk/run narrative spine as the readout, but keeps the rubric scores out of the main slides and confined to an appendix, so the story stays the focus and the scoring stays available for anyone who wants to check the math. All three outputs come from the same scored data, so the internal plan, the written narrative, and the client-facing deck never drift apart.
